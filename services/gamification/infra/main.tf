# Infra propia de gamification (doc 06 s5.3): state independiente; consume los
...
terraform {
  ...
  backend "s3" {
    ...
    key            = "dev/services/gamification/terraform.tfstate"
    region         = "us-east-1"
...
provider "aws" {
  region = "us-east-1"
  default_tags {
    tags = { Project = "edtech", Env = "dev", Service = "gamification", ManagedBy = "terraform" }
  }
}
...
module "servicio" {
  ...
  entorno           = "dev"
  nombre            = "gamification"
  imagen            = "${local.cuenta}.dkr.ecr.us-east-1.amazonaws.com/edtech/gamification:${var.image_tag}"
  ...
  env = {
    ...
    DB_SECRET_NAME         = "edtech/dev/db/gamification"
    QUEUE_URL              = data.terraform_remote_state.messaging.outputs.colas["gamification"].url
    QUEUE_CERTIFICADOS_URL = data.terraform_remote_state.messaging.outputs.colas["gamification-certificados"].url
    BUCKET_MEDIA           = data.terraform_remote_state.storage.outputs.bucket_media
    COGNITO_ISSUER         = data.terraform_remote_state.cognito.outputs.issuer
    LOG_LEVEL              = "info"
  }

  # Minimo privilegio (doc 07 s10): solo lo que gamification usa
  permisos = [
    {
    ...
    },
    {
      ...
      resources = [
        data.terraform_remote_state.messaging.outputs.colas["gamification"].arn,
        data.terraform_remote_state.messaging.outputs.colas["gamification-certificados"].arn,
      ]
    },
    {
      actions = ["secretsmanager:GetSecretValue"]
      resources = [
        data.terraform_remote_state.database.outputs.secretos_db["gamification"],
      ]
    },
    {
      actions   = ["s3:PutObject", "s3:GetObject"]
      resources = ["${data.terraform_remote_state.storage.outputs.bucket_media_arn}/certificados/*"]
    },
    {
    ...
    },
    {
      ...
      resources = ["arn:aws:rds-db:us-east-1:${local.cuenta}:dbuser:${data.terraform_remote_state.database.outputs.cluster_resource_id}/svc_gamification"]
    },
  ]
}
