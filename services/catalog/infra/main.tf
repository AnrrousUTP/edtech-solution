# Infra propia de catalog (doc 06 §5.3): state independiente; consume los
...
terraform {
  ...
  backend "s3" {
    ...
    key            = "dev/services/catalog/terraform.tfstate"
    region         = "us-east-1"
...
provider "aws" {
  region = "us-east-1"
  default_tags {
    tags = { Project = "edtech", Env = "dev", Service = "catalog", ManagedBy = "terraform" }
  }
}
...
module "servicio" {
  ...
  entorno           = "dev"
  nombre            = "catalog"
  imagen            = "${local.cuenta}.dkr.ecr.us-east-1.amazonaws.com/edtech/catalog:${var.image_tag}"
  ...
  env = {
    ...
    DB_SECRET_NAME           = "edtech/dev/db/catalog"
  ...
  # Mínimo privilegio (doc 07 §10): solo lo que catalog usa
  permisos = [
    {
    ...
    },
    {
      ...
      resources = [
        data.terraform_remote_state.database.outputs.secretos_db["catalog"],
    ...
    },
    {
      ...
      resources = ["arn:aws:rds-db:us-east-1:${local.cuenta}:dbuser:${data.terraform_remote_state.database.outputs.cluster_resource_id}/svc_catalog"]
    },
  ]
}
