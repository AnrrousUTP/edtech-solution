# Infra propia de enrollment-progress (doc 06 s5.3): state independiente; consume los
...
terraform {
  ...
  backend "s3" {
    ...
    key            = "dev/services/enrollment-progress/terraform.tfstate"
    region         = "us-east-1"
...
provider "aws" {
  region = "us-east-1"
  default_tags {
    tags = { Project = "edtech", Env = "dev", Service = "enrollment-progress", ManagedBy = "terraform" }
  }
}
...
module "servicio" {
  ...
  entorno           = "dev"
  nombre            = "enrollment-progress"
  imagen            = "${local.cuenta}.dkr.ecr.us-east-1.amazonaws.com/edtech/enrollment-progress:${var.image_tag}"
  ...
  env = {
    ...
    DB_SECRET_NAME            = "edtech/dev/db/enrollment"
    QUEUE_URL                 = data.terraform_remote_state.messaging.outputs.colas["enrollment"].url
    CATALOG_BASE_URL          = "http://${data.terraform_remote_state.compute.outputs.alb_dns}"
    INTERNO_TOKEN_SECRET_NAME = "edtech/dev/interno"
    COGNITO_ISSUER            = data.terraform_remote_state.cognito.outputs.issuer
    LOG_LEVEL                 = "info"
  }

  # Minimo privilegio (doc 07 s10): solo lo que enrollment-progress usa
  permisos = [
    {
    ...
    },
    {
      ...
      resources = [
        data.terraform_remote_state.messaging.outputs.colas["enrollment"].arn,
      ]
    },
    {
      actions = ["secretsmanager:GetSecretValue"]
      resources = [
        data.terraform_remote_state.database.outputs.secretos_db["enrollment"],
        data.terraform_remote_state.security.outputs.secreto_interno_arn,
      ]
    },
    {
    ...
    },
    {
      ...
      resources = ["arn:aws:rds-db:us-east-1:${local.cuenta}:dbuser:${data.terraform_remote_state.database.outputs.cluster_resource_id}/svc_enrollment"]
    },
  ]
}
